...
class NMMOWrapper(Wrapper):
    ...
    def step(self, actions):
        decisions = self._parse_action(actions, dead_agents=self._dead_agents)
    ...
    @staticmethod
    def _parse_action(actions, dead_agents=None):
        """
        decisions = {
            action.Attack: {
                action.Style: 0,
        ...
        for agent_id, act in actions.items():
            if dead_agents and agent_id in dead_agents:
                continue
            if act == 0:
                decisions[agent_id] = {}
            elif 1 <= act <= 3:
