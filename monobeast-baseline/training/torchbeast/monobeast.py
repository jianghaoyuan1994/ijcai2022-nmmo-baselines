...
import nmmo
import torch
from torch import logit, multiprocessing as mp
from torch import nn
from torch.nn import functional as F

from torchbeast.core import file_writer, prof, vtrace
from torchbeast.neural_mmo.config import BaselineConfig
...
def batch(env_output: Dict, filter_keys: List[str]):
    ...
    for agent_id, out in env_output.items():
        ...
        for key, val in out.items():
            if key in filter_keys: obs_batch[key].append(val)
...
def create_env(*args, **kwargs):
    return NMMOWrapper(nmmo.Env(config=BaselineConfig()))
...
if __name__ == "__main__":
    ...
    flags.num_agents = BaselineConfig().NENT
    main(flags)
