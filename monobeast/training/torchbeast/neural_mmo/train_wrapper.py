...
class FeatureParser:  # 环境obs解析
    ...
    feature_spec = {
        ...
        "now_time": spaces.Box(low=0, high=1024, shape=(1,), dtype=np.int64),
        "mine_loc": spaces.Box(low=0, high=256, shape=(2,), dtype=np.float32),
    }
    ...
    def parse(self, obs):
        ...
        for entity in obs.keys():
            ...
            now_time = np.array([max(obs_agents[:, 8])], dtype=np.int)
            ...
            while index < 100:
                ...
                if index == 0:
                    is_mine = 1
                    mine_loc = np.array([float(value[5]), float(value[6])])
                else:
                    is_mine = 0
                ...
                if value[2] == 0:
                    ...
                    attack_team = self.onehot_team[16] if value[2] < 0 else self.onehot_team[
                        int((value[2] - 1) // 8)]  # 0的时候应该全0？
                    attack_id = self.onehot_index[8] if value[2] < 0 else self.onehot_index[
                        int((value[2] - 1) % 8)]  # 0的时候应该全0？
                ...
                if team_in_ == 0:
                    team_in_ = reversed_team_id
                elif team_in_ == reversed_team_id:
                    team_in_ = 0
                ...
                loc_r = r_in - init_R
                loc_c = c_in - init_C
                assert 0 <= loc_c <= 14 and 0 <= loc_r <= 14, "{}-{}-{}-{}-{}".format(loc_r, init_R, loc_c, init_C,
                                                                                      obs[entity])
...
class TrainWrapper(Wrapper):
    ...
    def __init__(self, env: TeamBasedEnv) -> None:
        ...
        self.action_move_space = spaces.Discrete(5)
        self.action_attack_type_space = spaces.Discrete(4)
        self.action_attack_unit_space = spaces.Discrete(100)
    ...
    def reset_scripted_team(self, config):
        ...
        forage_num = np.random.randint(15 - random_num)
        print(random_num, forage_num, 15 - random_num - forage_num)
        ...
        for i in range(config.NPOP):
            if i == self.TT_ID:
                continue
            if self.TT_ID < i <= self.TT_ID + random_num:
                self._scripted_team[i] = RandomTeam(f"random-{i}", config)
            elif self.TT_ID + random_num < i <= self.TT_ID + random_num + forage_num:
                self._scripted_team[i] = ForageTeam(f"forage-{i}", config)
            elif self.TT_ID + random_num + forage_num < i <= self.TT_ID + 15:
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        while True:
            actions = {
                agent_id: {
                    "action_move": env.action_move_space.sample(),
                    "action_type": env.action_attack_type_space.sample(),
                    "action_unit_id": env.action_attack_unit_space.sample()
                }
